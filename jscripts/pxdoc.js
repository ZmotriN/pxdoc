...
/******************************************************
 *              Get Scaled Text Fontsize              *
 ******************************************************/
const getScaledTextFontSize = (text, width) => {
    const canvas = getScaledTextFontSize.canvas || (getScaledTextFontSize.canvas = document.body.create('div', 'fontsize-width-calc'));
    canvas.innerText = text;
    if(canvas.clientWidth <= width) return 60;
    return (width * 60 / (canvas.clientWidth + 1)).toFixed(0);
}
...
const lang_cache = 'lang_cache_3_' + meta_lang;
...
app.component('grostitre', {
    data() {
        if(typeof this.$slots.default === 'undefined') return null;
